...
summary(dat <- readRDS("dat_Mackenzie.rds"))

# split into used and available sets
...
# remove veg categories with < 1% cover: veg_100, veg_35, and veg_40
dat.available <- dat.available %>% select(-veg_100, -veg_35, -veg_40)
dat.used <- dat.used %>% select(-veg_100, -veg_35, -veg_40)
...
points(longitude ~ latitude, data = dat.used,
       subset = (ID == "MB-21-04"), pch = 19, cex = 1, col = "red")
points(longitude ~ latitude, data = dat.used,
       subset = (ID == "MB-21-15"), pch = 19, cex = 1, col = "blue")
points(longitude ~ latitude, data = dat.used,
       subset = (ID == "MB-21-16"), pch = 19, cex = 1, col = "green")
# all's well - keep all individuals 
...
results.ML <- array(dim = c(length(unique.IDs), 15))
results.SE <- array(dim = c(length(unique.IDs), 15))
for (i in 1:length(unique.IDs)) { # loop over individuals
  ...
  # fit a Binomial while weighing up available points
  # as recommended in Fieberg et al. 2021
